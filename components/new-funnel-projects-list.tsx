...
export function NewFunnelProjectsList() {
  const [projects, setProjects] = useState<NewFunnelProject[]>([]);
  ...
  const handleDelete = async (
    ...
    try {
      const res = await fetch(
        `/api/agents/new-funnel/funnels/${project.id}`,
        { method: "DELETE" },
      );
  ...
  useEffect(() => {
    ...
    fetch("/api/agents/new-funnel/funnels?list=true")
      .then((res) => res.json())
      .then((data) => {
        const loaded = (data.funnels ?? []) as NewFunnelProject[];
        setProjects(loaded);
        setStatus(
          loaded.length > 0
    ...
    <div className="mx-auto max-w-5xl">
      <div className="mb-8 flex items-center justify-between gap-4">
        ...
        <Button asChild size="default" className="gap-2 shadow-sm">
          <Link href="/agents/new-funnel/projects/new">
      ...
      {projects.length === 0 && !isLoading ? (
        <div className="flex flex-col items-center justify-center rounded-2xl border-2 border-dashed border-border/60 bg-gradient-to-b from-muted/30 to-muted/10 py-20 text-center">
          ...
          <p className="mt-1.5 max-w-sm text-sm text-muted-foreground">
            Create your first CRO project to optimize copy, bridge funnel structures, or apply conversion patterns.
          </p>
        ...
        <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
          {filteredProjects.map((project, i) => {
            ...
            return (
              <Link
                key={project.id}
                href={`/agents/new-funnel/projects/${project.id}`}

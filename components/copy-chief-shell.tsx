...
export function CopyChiefShell({
  children,
}: {
    ...
    <SidebarProvider defaultOpen={false}>
          ...
          <SidebarGroup>
            ...
            <SidebarGroupContent>
              <SidebarMenu>
                {navItems.map((item) => {
                  const active =
                    item.href === "/agents/copy-chief"
                      ? pathname === item.href
                      : item.href === "/agents/copy-chief/projects"
                        ? pathname === item.href
      ...
      <SidebarInset>
        <header className="flex h-12 shrink-0 items-center gap-2 border-b bg-background px-4 md:hidden">
          <SidebarTrigger />
          <span className="font-medium">Copy Chief</span>
        </header>
